...
function setupPreviewCamera() {
    ...
    console.log('[Preview] Фактическая позиция персонажа:', actualPlayerPos);
    
    const camOffset = 2.5;
    const headingRad = (player.getHeading() * Math.PI) / 180;
    const rightShift = 1.5;
    
    const camPos = new mp.Vector3(
        actualPlayerPos.x + Math.sin(headingRad) * camOffset - rightShift,
        actualPlayerPos.y + Math.cos(headingRad) * camOffset,
        actualPlayerPos.z + 0.6
    );
    
    console.log('[Preview] Позиция камеры:', camPos);
